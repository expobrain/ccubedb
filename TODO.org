...
** DONE big data test
  ...
  - [X] time biggest data
    ...
    - [X] COUNT all group by app_version:
      ...
      fast build (-O2 -flto -march=native): 0.634 seconds

      fastest build (-Ofast -flto -march=native): 0.640 seconds

    - [X] PCOUNT all group by app_version:

      slow build: 3.073 s

      fast build (-O2 -flto -march=native): 0.672 seconds

      fastest build (-Ofast -flto -march=native):  0.642 seconds

* TODO [0/8] Prepare for test launch (the project works in background)
...
** TODO null values for start/stop partitions
...
** TODO a big test suite based on counts of real event data
