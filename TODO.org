...
* TODO [7/13] Prepare for test launch (the project works in background)
...
** TODO [2/6] backend DSs
  ...
  - [ ] instead of sending data to the client while still building dss -> prepare the data first,
    then send. If the error condition happens - abort and drop the data
